...
const sections = ['check', 'services', 'process', 'contact'];
const serviceIcons = [ShieldCheck, Search, Gauge, Code2];
...
export default function App({ lang = 'de', page = 'home' }) {
  ...
  return <>
    ...
    <header><div className="container header-inner"><Brand/>
      ...
      <div className="header-actions"><a className="language" href={`/${lang === 'en' ? 'de' : 'en'}/${pageSuffix}`} aria-label={lang === 'en' ? 'Auf Deutsch wechseln' : 'Switch to English'}>{lang.toUpperCase()}<ChevronDown size={13}/></a><a className="button header-cta" href={`/${lang}/#check`}>{t.check}</a><button className="menu-button" aria-label={menu ? t.close : 'Menu'} aria-expanded={menu} onClick={() => setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button></div>
    </div></header>
    <main id="main">{page !== 'home' ? <article className="container legal"><a href={`/${lang}/`} className="text-link">← {t.back}</a><h1>{page === 'privacy' ? t.privacyTitle : t.termsTitle}</h1>{(page === 'privacy' ? t.privacyText : t.termsText).map(text => <p key={text}>{text}</p>)}<button className="button" onClick={openContact}>{t.contact}<ArrowRight size={17}/></button></article> : <>
      <section className="hero" id="check"><img className="hero-image" src="/alps.webp" alt="" fetchPriority="high"/><div className="hero-shade"/>
        ...
        </div><Report t={t} report={report} isDemo={isDemo} busy={busy} stage={job?.stage || 'queued'} onPlan={showPlan}/></div>
      </section>
    ...
    <footer className="container"><Brand/><span className="footer-line">Secure. Visible. Ready to grow.</span><div><button onClick={openContact}>{t.contact}</button><a href={`/${lang}/privacy/`}>{t.privacy}</a><a href={`/${lang}/terms/`}>{t.terms}</a></div></footer>
    <dialog ref={dialog} onCancel={() => setContact(false)} onClick={event => {if(event.target===dialog.current) setContact(false);}}><div className="contact-dialog"><button className="dialog-close" aria-label={t.close} onClick={() => setContact(false)}><X size={22}/></button><div className="eyebrow">CYBERALPS</div><h2>{t.requestTitle}</h2><p>{t.requestIntro}</p>
      {leadStatus === 'done' ? <div className="success" role="status"><CheckCircle2 size={28}/><p>{t.success}</p><button className="button" onClick={() => setContact(false)}>{t.close}</button></div> : <form onSubmit={sendLead} className="contact-form"><div className="form-row"><label>{t.name}<input name="name" autoComplete="name" minLength={2} maxLength={100} required/></label><label>{t.email}<input name="email" type="email" autoComplete="email" maxLength={254} required/></label></div><label>{t.website}<input name="url" autoComplete="url" defaultValue={url} maxLength={2048}/></label><label>{t.message}<textarea name="message" rows={4} minLength={10} maxLength={3000} required/></label><label className="honeypot" aria-hidden="true">Leave empty<input name="website" autoComplete="off" tabIndex={-1}/></label><label className="checkbox"><input type="checkbox" name="consent" required/><span>{t.contactConsent} <a href={`/${lang}/privacy/`}>{t.privacy}</a></span></label>{leadError && <p role="alert" className="error">{leadError}</p>}<button className="button" disabled={leadStatus === 'sending'}>{leadStatus === 'sending' ? t.sending : t.send}<ArrowRight size={18}/></button></form>}
    </div></dialog>
  </>;
}
